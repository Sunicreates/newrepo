...
app.get('/api/projects', authenticateToken, async (req, res) => {
  try {
    ...
    // For regular users, return only their projects
...
app.post('/api/projects', authenticateToken, async (req, res) => {
  try {
    ...
    const project = new Project({
      ...
      url: req.body.url || ''// Use name or email username as fallback
    });
    await project.save();
...
app.put('/api/projects/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin') {  // Changed from email check to role check
    ...
    const updateData = {
      ...
      techStack: req.body.techStack || existingProject.techStack
    };
    console.log('Merged update data:', updateData);

    const project = await Project.findOneAndUpdate(
      query,  // Use the same query here
      updateData,
      { new: true, runValidators: true }
    );

    if (!project) {
      return res.status(404).json({ error: 'Project update failed' });
    }
